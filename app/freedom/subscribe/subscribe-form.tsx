...
export default function SubscribeForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/mailerlite", {
        ...
        body: JSON.stringify({
          email: trimmedEmail,
          source: "bedo_freedom_subscribe_page",
        }),
      })
  ...
  return (
    <div className="theme-financial min-h-screen bg-background relative flex flex-col overflow-hidden">
      {/* Background grids and glows */}
      <div className="absolute inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_15%_20%,oklch(0.7_0.25_145_/_0.08)_0%,transparent_40%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_85%_80%,oklch(0.7_0.25_145_/_0.04)_0%,transparent_45%)]" />
        <div 
              ...
              <div className="text-center py-6 space-y-6 animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground text-sm md:text-base max-w-sm mx-auto">
                    Thanks for subscribing to <strong className="text-foreground font-semibold">The Freedom Blueprint</strong>. Keep an eye on your inbox for the next edition.
                  </p>
                </div>
                <div className="pt-2">
                  <Link href="/" passHref legacyBehavior>
                    <Button className="w-full sm:w-auto px-6 font-semibold bg-primary hover:bg-primary/95 text-primary-foreground">
                      Explore Bedo Freedom
                    </Button>
                  </Link>
                </div>
              </div>
