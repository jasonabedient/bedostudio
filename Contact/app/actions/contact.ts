...
export async function submitContactForm(
  ...
  console.log('[Contact Form - Subfolder] Received submission:', {
    name,
    email,
    subject,
    consent,
  ...
  if (Object.keys(errors).length > 0) {
    console.warn('[Contact Form - Subfolder] Validation failed:', errors)
    return {
      success: false,
  ...
  if (!userSmtp || !passSmtp) {
    ...
    console.error(`[Contact Form - Subfolder] Configuration error: ${errorMsg}`)
    return {
      success: false,
  ...
  try {
    console.log(`[Contact Form - Subfolder] Preparing SMTP transport through ${hostSmtp}:${portSmtp}`)
    ...
    console.log('[Contact Form - Subfolder] Sending mail...')
    const info = await transporter.sendMail(mailOptions)
    console.log('[Contact Form - Subfolder] Mail sent successfully. Message ID:', info.messageId)
    ...
    console.error('[Contact Form - Subfolder] SMTP dispatch exception:', error)
    return {
      success: false,
